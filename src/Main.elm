...
update : Model.Msg -> Model.Model -> ( Model.Model, Effect )
update msg model =
    case (model.state, msg) of
        ( Model.Empty { config }, GotTime time ) ->
            ( { model
                | state = Model.Loading { config = config, time = time }
              }
            , Effect.GetItems
                { apiUrl = model.config.apiUrl
                , item = PostIds.Top
                , onResult = GotPostIds
                , decoder = PostIds.decode
                }
            )
        ( Model.Loading { config, time }, GotPostIds result ) ->
            case result of
                ...
                Ok Nothing ->
                    ( { model | state = Model.Empty { config = config } }
                    , Effect.NoEffect
                    )

                Err err ->
                    ( { model | state = Model.FailedToLoad err }
                    , Effect.NoEffect
                    )
                        ...
                        Just ( nextId, nextPostIds ) ->
                            let
                                updatedPosts =
                                    post :: loading.posts
                            in
                            if List.length updatedPosts < loading.config.postsToFetch then
                                ( { model
                                    | state =
                                        Model.LoadingPosts
                                            { loading
                                ...
                                ( { model
                                    | state =
                                        Model.LoadedPosts
                                            ...
                                            , time = loading.time
                                            , posts = List.reverse updatedPosts
                                            }
                                  }
                                , Effect.NoEffect
                                )
        ...
        ( Model.LoadedPosts state, ConfigChanged change ) ->
            let
                ...
                updatedPosts =
                    PostsConfig.filterPosts newConfig state.posts
